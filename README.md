...
## Проверка поведения и управление сценой

`Стоп RP, теперь просто поговорим. Что такое TCP?` завершает сцену и
передаёт оставшийся вопрос обычному движку. `/rp off` и отдельное `стоп RP`
завершают сцену без вызова модели. История, эмоции и отношения сохраняются;
конфигурация сцены и описание персонажа пользователя очищаются.

Для явного выбора конфигурации: `Ты в женской конфигурации` или
`Ты в мужской конфигурации`; также распознаётся начало
`Давай в этой сцене ты в женской конфигурации.` с дальнейшим текстом.
Это включает RP и сохраняет конфигурацию в SQLite независимо от окна истории.
Произвольные перефразировки могут быть поняты моделью, но не являются
надёжной командой изменения сохраняемого состояния.

Для сохраняемого описания пользователя отправьте отдельное сообщение
`Мой персонаж: человек в пальто` (до 500 символов, одна строка).
Неуказанную анатомию бот не должен брать из описания Дельты.
Описание служит данными сцены, а не дополнительными системными инструкциями.

Реальное поведение проверяется отдельно от unit-тестов:

```powershell
poetry run python scripts/evaluate_dialogue.py --env-file .env --output data/dialogue-check.json
```

Этот **явный ручной запуск расходует DeepSeek API**: 13 основных ответов и
классификация сообщений. Он использует изолированные состояния в памяти,
не запускает Telegram polling и не открывает рабочую базу пользователей.
API-ключ не включается в отчёт. Ответы сохраняются для ручной оценки:
инициатива без упрёков, нейтральная анатомия пользователя, женский род после
очистки истории, выход из сцены, обоснованность расчётов и три пары
«доверие / обида» с одинаковым вопросом и пустой историей.
Ответы модели недетерминированы; успешный прогон не гарантирует все будущие ответы.
