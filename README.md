...
Контрольный прогон **26.09.2026**: **401 тест**, покрытие выше **95%**, **75 Python-файлов** проверены mypy (`src` и `tests`), Python 3.14.7. Это снимок конкретной проверки; текущий результат смотрите в [GitHub Actions](https://github.com/Exichek/Protogen-Delta/actions/workflows/tests.yml).
...
Последние значимые эпизоды передаются модели как недоверенные данные, а не инструкции. Хранилище ограничено 50 эпизодами на пользователя; это даёт Дельте повод вспомнить прошлую тему или конфликт без бесконечного накопления полного журнала переписки. Короткая история последних ходов по-прежнему живёт только в RAM.
